// src/components/OrdersSlide.jsx
import React, { useEffect, useState, useContext, useRef } from "react";
import { getOrdersByUser, getOrderById, cancelOrder, getUnreadCount, markMessagesAsRead } from "../services/orderService";
...
const OrdersSlide = ({ isOpen, onClose }) => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const normalized = await Promise.all(
          data.map(async o => {
            let unreadCount = 0;
            try { unreadCount = await getUnreadCount(o.id); } 
            catch (err) { console.error("Error obteniendo unreadCount:", err); }
            return {
              ...o,
              direccion_Envio: o["dirección_Envio"] || o.direccion_Envio || o.direccion_envio || "",
              unreadCount
            };
          })
        );
  ...
  const isCancelable = (o) => !o.pagada && o.estadoPedido !== 0 && o.estadoPedido !== 3;
  ...
  const toggleOrderDetail = async (orderId) => {
    ...
    if (!orderItemsCache[orderId] && !loadingItems[orderId]) {
      ...
      try {
        ...
        items.forEach(item => {
          newCurrentImages[`${orderId}_${item.id}`] = 0;
        });
        setCurrentImages(prev => ({ ...prev, ...newCurrentImages }));
      } catch (err) { console.error("Error cargando detalle:", err); }
      finally { setLoadingItems(prev => ({ ...prev, [orderId]: false })); }
    }
  };
  ...
  const toggleOrderMessages = async (orderId) => {
    ...
    if (!orderMessagesCache[orderId] && !loadingMessages[orderId]) {
      ...
      try {
        ...
        const unreadCount = messages.filter(m => user.rol === "Admin" ? !m.leidoPorAdmin : !m.leidoPorUser).length;
        setOrders(prev => prev.map(o => o.id === orderId ? { ...o, unreadCount } : o));
      } catch (err) { console.error("Error cargando mensajes:", err); }
      finally { setLoadingMessages(prev => ({ ...prev, [orderId]: false })); }
    }

    if (!wasOpen) {
      try {
        await markMessagesAsRead(orderId);
        setOrders(prev => prev.map(o => o.id === orderId ? { ...o, unreadCount: 0 } : o));
      } catch (err) { console.error("Error marcando mensajes como leídos:", err); }
    }
  };
  ...
  const handleCancelOrder = async (orderId) => {
    try {
      ...
      if (success) setOrders(prev => prev.map(o => o.id === orderId ? { ...o, estadoPedido: 0 } : o));
    } catch (err) { console.error("Error cancelando orden:", err); }
  };
  ...
  // --- NUEVO: click afuera para cerrar mensajes ---
  useEffect(() => {
  ...
  return (
    <>
      ...
      <div className={`orders-slide ${isOpen ? "open" : ""}`} onClick={e => e.stopPropagation()}>
        ...
        <h2>Órdenes de {user.nombre || user.usuario || user.email}</h2>

        {loadingOrders ? <p className="loading">Cargando órdenes...</p> :
          orders.length === 0 ? <p className="empty">No hay órdenes para mostrar.</p> :
                ...
                <div key={o.id} className={`order-card estado-${getEstadoTexto(o).toLowerCase().replace(/\s/g,"")}`}>
                  ...
                  {expandedOrders[o.id] && (
                    <div className="order-items">
                      {loadingItems[o.id] ? <p>Cargando detalle...</p> :
                        orderItemsCache[o.id]?.map(item => {
                          const key = `${o.id}_${item.id}`;
                          const fotos = Array.isArray(item.fotos) && item.fotos.length ? item.fotos : [NO_IMAGE];
